...
const init = async () => {
	const res = await getTags(props.tagType)
	tagList.value = []
	res.LabelValues.forEach((item) => {
		const res = {
			value: item.ColIndex,
			name: item.ColName,
			tags: [],
		}
		item.ColValues.forEach((t) => {
			if (t.IsDefault) {
				tagMap.value.set(t.ReportColLabelID, t.ReportColLabelDesc)
			}
			res.tags.push({
				name: t.ReportColLabelDesc,
				value: t.ReportColLabelID,
			})
		})
