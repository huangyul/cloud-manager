  ...
  <div>
    ...
    <div class="action">
      ...
      <button class="btn btn-green flex flex-col-center" @click="openDialog()">
        <el-icon class="mr6"><Plus /></el-icon>新建转仓单
      </button>
    </div>
      ...
      >
        ...
        >
          <template #default="{ row }">
            <span>{{ statusMap.get(row.status) }}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="业务日期"
...
const onDialogConfirm = async () => {
  switch (confirmDialogType.value) {
    case "submit": {
      // await submitTransferboune({ bill_id: currentItem.value.id })
      doSearch();
      ElMessage.success("提交成功");
      break;
    }
    case "delete": {
      // await deleteOrder({ bill_id: currentItem.value.id })
      doSearch();
...
const doSearch = async () => {
  // const res = await getTransferbouneOrderList({
